...
const BuyCredit = () => {
  ...
  const initPay = async (order) => {
    const options = {
      ...
      // UPDATE THIS HANDLER SECTION:
      handler: async (response) => {
    try {
        const token = await getToken();
        
        // This sends the razorpay_order_id to the function we just fixed above
        ...
        if (data.success) {
            await loadCreditsData(); // Refresh the Navbar
      ...
      <div className='flex flex-wrap justify-center gap-6 text-left'>
        {plans.map((item, index) => (
          <div className='bg-white drop-shadow-sm border rounded-lg py-12 px-8 text-gray-700 hover:scale-105 transition-all duration-500' key={index}>
              ...
              <p className='mt-3 font-se'>{item.id}</p>
